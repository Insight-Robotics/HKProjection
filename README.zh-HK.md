...
- **七參數的出處** — 示意圖點名了參數文件（《Geodetic Datum Transformation and
  ...
  頁的獨立數值及 EPSG:1825 作驗證，兩者完全吻合。若用於測量作業，請先與地政總署
  自行發佈的版本核對 — 該參數組連同《說明》一併發佈於
  <https://www.geodetic.gov.hk/tc/gi/refdoc.htm>。
...
上述文件版權屬香港特別行政區政府所有，在此僅供參考之用。文件的最新版本連同坐標
轉換參數，均發佈於 <https://www.geodetic.gov.hk/tc/gi/refdoc.htm>。

本專案為獨立實作，並未獲地政總署認可。**在用於測量、法定或攸關安全的工作之前，
請先與官方來源核對。**
...
本專案自行撰寫的軟體 — Python 原始碼、工具程式及說明文件 — 以 **MIT 授權條款**
釋出，詳見 [`LICENSE`](LICENSE)。

MIT 授權**並不涵蓋** `Reference/` 內的地政總署刊物：該等文件版權仍屬香港特別行政
區政府所有並保留一切權利；亦不涵蓋 `hk_height_model.csv` 所依據的測量原始資料。
完整的授權範圍，以及如需轉載時的注意事項，請參閱 [`NOTICE.md`](NOTICE.md)。
